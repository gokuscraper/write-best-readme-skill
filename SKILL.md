...
- **Mole 式生动**:H1 标题必须配一个「生物」emoji(见硬性规则);Support 捐赠段落**固定用宠物投喂式措辞**:不说"支持项目/请我喝咖啡",一律写"请我的宠物吃饭",如 `如果 Mole 帮到了你,给个 star 或请我的两只猫(汤圆、可乐)吃 [罐头 🥩](链接)`;宠物种类/名字在第 2 步问清(默认:两只猫),名字没给就不写,不编造;CLI 项目用终端输出代码块展示效果
- **Mole 式居中头部**:
  ```html
  <div align="center">
    <h1>🐹 Mole</h1>
...
- 宠物投喂措辞:宠物种类/名字(默认两只猫,名字没给不写),投喂物(默认罐头 🥩)
- 对比表竞品(扫描识别的同行,或用户指定)
