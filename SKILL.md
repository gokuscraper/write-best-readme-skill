...
- **Mole 式生动**:H1 标题必须配一个「生物」emoji(见硬性规则);Support 捐赠段落**固定用宠物投喂式措辞**:不说"支持项目/请我喝咖啡",一律按模板写(默认两只猫:汤圆、饺子,投喂物罐头 🥩):
  `我养了两只猫,汤圆和饺子。如果你觉得 X 给你的生活带来了快乐,你可以喂它们 [罐头食品 🥩](捐赠链接)。`
  宠物种类/名字在第 2 步确认(默认:两只猫,汤圆、饺子),名字没给就不写,不编造;CLI 项目用终端输出代码块展示效果
- **Mole 式居中头部**:
  ```html
  <div align="center">
    <h1>🐹 Mole</h1>
...
- 宠物投喂措辞:宠物种类/名字(默认两只猫:汤圆、饺子),投喂物(默认罐头食品 🥩)
- 对比表竞品(扫描识别的同行,或用户指定)
